...
长文本框在下：位于按钮框架下方，独占一行，显示日志信息

用户可以通过按钮打开文件夹，生成前按钮是灰的，只有生成后按钮才亮

## 思路

使用python语言，提取（移动）PDF文件,制作GUI界面

用户通过资源管理器选择要提取PDF的文件夹，文本框显示选择的路径，

PDF文件提取后保存的文件夹，创建于用户选择的"提取PDF的文件夹"路径一样，文件夹名称与用户选择的"提取PDF的文件夹"的名称一样，再加上"_PDF"，其中创建文件夹下的文件结构不变。

提取（移动）PDF文件后，用户可以通过按钮打开创建的文件夹，生成前按钮是灰的，只有生成后按钮才亮
...
```bash
pyinstaller --onefile --windowed PDF_GUI.py
```
